...
export function wordHighlightClass(isActive: boolean, isMockHighlight = false): string {
  if (isActive) {
  ...
  if (isMockHighlight) {
    return `${wordBase} text-[#d0d2dc] underline decoration-1 decoration-accent/40 underline-offset-[3px] hover:bg-accent/[0.07] hover:text-zinc-50 hover:decoration-accent/60`;
  }
